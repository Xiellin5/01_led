...
extern const uint8_t human_fall_detect_espdl[] asm("_binary_human_fall_detect_espdl_start");
static const char *path = (const char *)human_fall_detect_espdl;
...
{
    switch (model_type) {
    case model_type_t::ESPDET_PICO_224_224_HUMAN_FALL:
...
void ESPDetDetect::load_model()
{
    switch (m_model_type) {
    case model_type_t::ESPDET_PICO_224_224_HUMAN_FALL:
#if CONFIG_FLASH_ESPDET_PICO_224_224_HUMAN_FALL || CONFIG_HUMAN_FALL_DETECT_MODEL_IN_SDCARD
        m_model = new espdet_detect::ESPDet("espdet_pico_224_224_human_fall.espdl", m_score_thr[0], m_nms_thr[0]);
#else
        ESP_LOGE("human_fall_detect", "espdet_pico_224_224_human_fall is not selected in menuconfig.");
#endif
        break;
    }
}
